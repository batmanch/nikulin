...
public class p73 {
    public static void main(String[] args) {
        try {
            File folder = new File("C:\\Users\\DALev\\OneDrive\\Рабочий стол\\My");
            if (!folder.exists()) folder.mkdir();


            File f1 = new File("C:\\Users\\DALev\\OneDrive\\Рабочий стол\\My\\rec_RAF.txt");
...
//----ЗАПИСЬ ИНФОРМАЦИИ О СОТРУДНИКАХ В ФАЙЛ----
            ...
            for (int i = 0; i < kol; i++) {
                ...
                rf.seek(rf.length());
                rf.writeUTF(fam);
                for (int j = 0; j < 20 - fam.length(); j++) rf.writeByte(1);

                System.out.print("Введите его должность => ");
                doljnost = sc.next();
                rf.writeUTF(doljnost);
                for (int j = 0; j < 20 - doljnost.length(); j++) rf.writeByte(1);
...
//----ЧТЕНИЕ ИНФОРМАЦИИ О СОТРУДНИКАХ ИЗ ФАЙЛА----
            ...
            for (int i = 0; i < kol; i++) {
                fam = rf.readUTF();
                for (int j = 0; j < 20 - fam.length(); j++) rf.readByte();
                doljnost = rf.readUTF();
                for (int j = 0; j < 20 - doljnost.length(); j++) rf.readByte();
                oklad = rf.readInt();
